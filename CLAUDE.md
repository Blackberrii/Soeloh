...
- **HorizonsRP** — a Garry's Mod DarkRP server. This is the one concrete active
  project. Real features: economy tuning, custom jobs, custom HUD, casino/coinflip
  systems, a "Credits" premium currency. (E2/Wiremod scripting was listed here
  before but isn't a current feature — don't reintroduce it.)
  ...
  - Live status: `scripts/query_status.py` queries the server via A2S and writes
    ...
    on a cron. The site fetches `status.json` client-side to show online/offline
    + player count next to the HorizonsRP heading.
...
- Static site, meant for GitHub Pages: `index.html` (hub — hero, about, HorizonsRP,
  Active Projects, music teaser, links), `music.html` (full Suno catalog), and
  ...
  `.card`/`.btn`/`.pill`, etc.) is duplicated between `index.html` and
  `music.html`. If a design-token or nav change is made in one, check whether it
  needs to be mirrored in the other.
